...
import type { VariableInfo, FrequencyResult } from '../../types/statistics';
import { getFrequencies } from '../../services/statistics-service';
import { useStats } from '../StatsContext';

interface FrequenciesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  isDarkMode: boolean;
  variables: VariableInfo[];
  selectedVariables: string[];
  onRun: (result: FrequencyResult[]) => void;
}

const FrequenciesDialog: React.FC<FrequenciesDialogProps> = ({
  isOpen,
  onClose,
  isDarkMode,
  variables,
  ...
  // Factors/categorical variables for frequency analysis
  const factorVars = useMemo(() => variables.filter(v => v.type === 'factor' || v.type === 'character'), [variables]);
  ...
  const handleRun = async () => {
    ...
    setError(null);
    try {
      const results = await getFrequencies(sessionId, localSelected);
      onRun(results);
      onClose();
    } catch (err) {
      ...
      <div className={`${cardStyle} animate-dialog-content`}>
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-500/30">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l-5-5m0 0l5-5m-5 5h12" />
            </svg>
          </div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Frequencies</h2>
        </div>
        ...
        <div className="mb-5">
          ...
          <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto border border-slate-200 dark:border-slate-700 rounded-lg p-3 bg-slate-50 dark:bg-slate-900/50">
            ...
            {factorVars.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400 italic">No categorical variables</p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className={labelStyle}>
              <input
                ...
                onChange={() => setOptions(prev => ({ ...prev, frequency: !prev.frequency }))}
                className="rounded text-emerald-500"
              />
            ...
            <label className={labelStyle}>
              <input
                type="checkbox"
                checked={options.percent}
                onChange={() => setOptions(prev => ({ ...prev, percent: !prev.percent }))}
                className="rounded text-emerald-500"
              />
            ...
            <label className={labelStyle}>
              <input
                type="checkbox"
                checked={options.validPercent}
                onChange={() => setOptions(prev => ({ ...prev, validPercent: !prev.validPercent }))}
                className="rounded text-emerald-500"
              />
              <span className="text-sm">Valid Percent</span>
            </label>
            <label className={labelStyle}>
              <input
                type="checkbox"
                checked={options.cumulative}
                onChange={() => setOptions(prev => ({ ...prev, cumulative: !prev.cumulative }))}
                className="rounded text-emerald-500"
              />
