              ...
              return (
                <div key={groupKey}>
                  ...
                  {!isCollapsed && groupItems.map(item => (
                    <div key={item.id} className={`px-3 py-3 ${itemBg} border-l-2 border-emerald-500 transition-all duration-150 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10`}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{item.title}</span>
                          <span className={`text-xs px-1.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 font-medium`}>{item.timestamp}</span>
                        </div>
                        <button
                          onClick={() => removeItem(item.id)}
                          className={`text-xs w-6 h-6 flex items-center justify-center rounded-full ${mutedText} hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-all duration-150`}
                          title="Remove"
                        >
                          ✕
                        </button>
                      ...
                      {item.plotHtml ? (
                        <div
                          className="mt-1 overflow-auto rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900"
                          style={{ maxHeight: '400px' }}
                          dangerouslySetInnerHTML={{ __html: item.plotHtml }}
                        />
                      ) : item.tableData ? (
                        <div className="mt-1 overflow-auto rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">{renderTable(item.tableData)}</div>
                      ) : null}
                    </div>
                  ))}
                </div>
